...
export function AppShell() {
  ...
  useEffect(() => { if (!open) return undefined; sidebar.current?.querySelector('a')?.focus(); const close=(event)=>{if(event.key==='Escape'){setOpen(false);menuButton.current?.focus();}};document.addEventListener('keydown',close);return()=>document.removeEventListener('keydown',close); }, [open]);
  useEffect(() => {
