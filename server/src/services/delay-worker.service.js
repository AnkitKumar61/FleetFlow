...
import { emitAdminAlert } from './realtime.service.js';
...
export async function processDelayCheck(deliveryId) {
  ...
  const notification = await Notification.findOneAndUpdate(
    { key },
    { $setOnInsert: { key, audienceRole: 'admin', type: 'delivery_delayed', delivery: delivery._id, message: `${delivery.trackingNumber} is past its expected delivery time.` } },
