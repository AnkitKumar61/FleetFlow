...
function AuthLayout({ mode }) {
  ...
  return <main className="auth-page">
    ...
    <section className="auth-form-wrap"><form className="auth-form" onSubmit={submit}><div><h2>{mode === 'login' ? 'Sign in to dispatch' : 'Create a customer account'}</h2><p>{mode === 'login' ? 'Use a demo account or your FleetFlow credentials.' : 'Request and track deliveries from one place.'}</p></div>
      ...
      <button className="button" disabled={busy}>{busy ? 'Please wait…' : mode === 'login' ? 'Enter operations' : 'Create account'} <ArrowRight /></button>
