...
resourceRouter.post('/drivers', authorize('admin'), validate({ body: z.object({ userId: z.string().regex(/^[a-f\d]{24}$/i), licenseNumber: z.string().min(3).max(30), licenseExpiresAt: z.coerce.date(), status: z.enum(['available', 'offline']).default('offline') }) }), asyncHandler(controller.createDriver));
resourceRouter.patch('/drivers/:id', authorize('admin'), validate({ params: id, body: z.object({ status: z.enum(['available', 'offline']).optional(), isActive: z.boolean().optional(), licenseExpiresAt: z.coerce.date().optional() }) }), asyncHandler(controller.updateDriver));
