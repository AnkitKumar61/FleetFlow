...
export function Loading({ label = 'Loading current data…' }) { return <div className="state"><LoaderCircle className="spin" aria-hidden="true"/><p>{label}</p></div>; }
...
export function ErrorState({ message, retry }) { return <div className="state state--error"><h2>Something needs attention</h2><p>{message}</p>{retry && <button className="button button--secondary" onClick={retry}>Try again</button>}</div>; }
