...
export default function DashboardPage() {
  const { user } = useAuth(); const [data, setData] = useState(null); const [deliveries, setDeliveries] = useState([]); const [error, setError] = useState('');
  const load = async () => { try { const requests = [api.get('/deliveries?limit=6')]; if (['admin','manager'].includes(user.role)) requests.push(api.get('/analytics/overview')); const [list, analytics] = await Promise.all(requests); setDeliveries(list.data.data); setData(analytics?.data.data ?? null); } catch (err) { setError(err.response?.data?.error?.message ?? 'Could not load current operations.'); } };
  useEffect(() => { load(); }, []);
  const heading = user.role === 'driver' ? 'Your delivery run' : user.role === 'customer' ? 'Your deliveries' : 'Today’s operating picture';
  if (error) return <ErrorState message={error} retry={load}/>;
  if (!deliveries.length && data === null) return <Loading/>;
  return <>
